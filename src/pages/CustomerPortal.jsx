          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '5px', flexWrap: 'wrap' }}>
            ...
            {showUnitDrop && hasMultiUnit && (
              <div className="unit-dropdown" onClick={e => e.stopPropagation()}>
                {ozelFiyatlar.map((f, i) => (
                  <button key={f.id || i} className="unit-dropdown-item" data-active={i === selectedIdx} onClick={(e) => { e.stopPropagation(); setSelectedIdx(i); setShowUnitDrop(false); }}>
                    {f.birim_adi || p.unit}
                  </button>
                ))}
              </div>
    ...
    <div className="product-card" {...hoverHandlers}>
      <div className="product-image-container">
        <div style={{ position: 'absolute', top: 8, right: 8, zIndex: 10 }}>
          ...
          {showUnitDrop && hasMultiUnit && (
            <div className="unit-dropdown unit-dropdown-card" onClick={e => e.stopPropagation()}>
              {ozelFiyatlar.map((f, i) => (
                <button key={f.id || i} className="unit-dropdown-item" data-active={i === selectedIdx} onClick={(e) => { e.stopPropagation(); setSelectedIdx(i); setShowUnitDrop(false); }}>
                  {f.birim_adi || p.unit}
                </button>
    ...
    <div className="page-container wide" style={{ paddingTop: 0 }}>
      ...
      {showBrandsView && (
        <div style={{ animation: 'fadeInUp 0.28s cubic-bezier(0.34,1.56,0.64,1)', padding: '0 0 16px', marginBottom: '8px' }}>
          ...
          {/* ARAMA & FİLTRE TOOLBAR - Marka sayfası için */}
          <div className="info-strip" style={{ marginTop: '20px', marginBottom: '12px' }}>
            <div className="info-left">
              <div className="search-discount-bar">
                <button
                  onClick={() => { setShowSearch(v => !v); if (!showSearch) setTimeout(() => searchInputRef.current?.focus(), 50); else setSearch(''); }}
      ...
      <style>{`
        ...
        .cp-small-action-btn { font-size: 18px; padding: 6px 8px; border-radius: 6px; border: 1px solid #e2e8f0; background: #fff; cursor: pointer; white-space: nowrap; display: inline-flex; align-items: center; justify-content: center; transition: all 0.2s; line-height: 1; min-width: 32px; }
        .cp-small-action-btn:hover { background: #f8fafc; border-color: #cbd5e1; }
        .cp-small-action-btn.pinned { background: rgba(34,197,94,0.06); border-color: rgba(34,197,94,0.4); }
        .cp-small-action-btn.clear { color: #dc2626; background: #fef2f2; border-color: #fecaca; }
        .cp-small-action-btn.clear:hover { background: #fee2e2; }
